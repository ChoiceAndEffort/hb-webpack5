<template>
	<div class="flex_row_space post_management hidden_y">
        <div class=" bf left">
            ...
            <el-tree class="expand-tree post_tree scroll_y" :data="data" :props="defaultProps" :filter-node-method="filterNode" default-expand-all ref="tree" @node-click="handleDepartmentRow"></el-tree>
        </div>
		<div class="position-list container-padding right">
			<div class="table-header">
				<div class="table-header-title">岗位管理 <span v-if="departmentActiveItemData.name">-{{departmentActiveItemData.name}}</span></div>
				<el-button type="primary" size="mini" @click="handleAddPostionFormShow" v-if="departmentActiveItemData.departmentId != ''">新增</el-button>
			</div>
			<el-divider></el-divider>
			<el-table ref="positionTable" :key="tableKey" :data="positionData" header-row-class-name="table-header-gray" class="btn-pointer"  v-loading="isLoading.positionList" style="width: 100%" row-key="positionId" lazy
                      :load="expandChange"
                      :tree-props="{children: 'children', hasChildren: 'name'}">
				<el-table-column prop="name" label="岗位名称"></el-table-column>
				<el-table-column prop="sort" label="排序"></el-table-column>
				<el-table-column prop="rank" label="职级" width="140"></el-table-column>
				<el-table-column fixed="right" label="操作" width="240">
					<template slot-scope="scope">
<!--						<el-button v-if="scope.row.roleId" type="text" size="small" @click.stop="handleRemove(scope.row)">删除</el-button>-->
<!--                        <el-button type="text" size="small" @click.stop="handleAdd(scope.row)">新增子级岗位</el-button>-->
						<el-button type="text" size="small" @click.stop="handleEditPositionFormShow(scope.row)">编辑</el-button>
		...
		<el-dialog :title="positionFormData.title+'岗位'" :visible.sync="isDialogShow.position">
			<el-form ref="positionForm" :model="positionFormData" :rules="positionRules" label-width="120px">
                ...
                <el-form-item label="上级岗位">
                    <el-cascader ref="cascader" v-model="positionFormData.pid" :show-all-levels="false" :options="levelpositionData" :props="props" style="width: 216px" @change="handleCascaderChange"></el-cascader>
                </el-form-item>
                <el-form-item label="是否负责人岗位" prop="isMaster">
                    <el-radio-group v-model="positionFormData.isMaster">
	...
	export default {
		...
		methods: {
            ...
            getProvence(node, resolve) {
                console.log(node)
                getPositionList({
                    departmentId: this.departmentActiveItemData.departmentId,
                    pid: node.value||0
                }).then((json) => {
			...
			// 部门管理 - 获取列表数据
			getDepartmentListData(data = {type: 0,enterpriseId:enterpriseId,status:1}) {
			...
			getPositionListData(departmentId) {
				...
				getPositionList({
                    ...
                    this.$set(this, 'levelpositionData', [{name: '顶级', positionId: '0'}]);
			...
			handleEditPositionFormShow(itemData) {
                ...
                this.positionFormData.isMaster = itemData.isMaster ? itemData.isMaster : 0
			...
			// 懒加载 - 岗位下角色列表
            expandChange(tree, treeNode, resolve) {
                getPositionList({
                    departmentId: this.departmentActiveItemData.departmentId,
                    pid: tree.positionId
                }).then(res=>{
            ...
            handleAdd(item){
                this.positionFormData = this.initPositionFormData();
                this.positionFormData.departmentId = this.departmentActiveItemData.departmentId;
                this.positionFormData.pid = item.positionId
                this.positionFormData.title = '新增';
                this.isDialogShow.position = true;
            },
			// 删除角色
